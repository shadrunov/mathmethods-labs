...
class Cryptosystem:
    ...
    def input_params(self):
        ...
        key = (
            input("Enter key. This is a short word in the same alphabet\n")
            .strip()
            .upper()
        )
    ...
    def decrypt(self, text: str) -> str:
        gamma = ( self.key * (len(text) // len(self.key) + 1) )[:len(text)]
        # print(gamma)
        ...
        res = [(sequence[i] - gamma[i]) % self.lang.modulo for i in range(len(sequence))]
        # print(res)
